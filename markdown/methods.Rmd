...
```{r data_processing_pipeline_flowchart, echo=FALSE, fig.height=10}
...
# input files
evidence_file <- boxGrob({glue("evidence file",
                           .sep = "\n")})
sample_annotation_file <- boxGrob({glue("sample annotation",
                           .sep = "\n")})
...
aggregate_psms <-  boxGrob({glue("aggregate peptide spectrum matches to
                                 peptides\n
                                 join multiple batches",
                          .sep = "\n")})

quality_control_2 <- boxGrob({glue("quality control 2",
                          .sep = "\n")})
qc2_content <- boxGrob("filter by reporter ion intensity cutoff\n
...
transform_data <- boxGrob({glue("transform data",
                          .sep = "\n")})
...
normalize_data <- boxGrob({glue("normalize data",
                          .sep = "\n")})
...
missing_value_handling <- boxGrob({glue("missing value handling",
                          .sep = "\n")})
...
batch_correction <- boxGrob({glue("batch correction",
                          .sep = "\n")})
...
dim_red <- boxGrob({glue("dimensionality reduction",
                          .sep = "\n")})
...
connectGrob(vert$quality_control_2, qc2_content, type = "horizontal", arrow_obj = arrow(angle=0))

connectGrob(vert$transform_data, transform_opts, type = "horizontal", arrow_obj = arrow(angle=0))

connectGrob(vert$normalize_data, normalize_opts, type = "horizontal", arrow_obj = arrow(angle=0))

connectGrob(vert$missing_value_handling, missing_value_opts, type = "horizontal", arrow_obj = arrow(angle=0))

connectGrob(vert$batch_correction, batch_correction_opts, type = "horizontal", arrow_obj = arrow(angle=0))
...
y_i = f(x_i)
$$
Depending on the distribution of the values in the observed expression set, different transformations can be applied to fulfill dependencies for statistical testing. In the shiny application various procedures are implemented and can be further expanded upon request from the user. The macrophage analysis done by Specht et. Al mentioned in their SCoPE2 publication citep\{Specht2021} uses the logarithm to the base 2 to spread a compacted distribution and the remove skewness in the dataset. This transformation was used as a reference when comparing methods. However using the logarithm to the base 10 can be easy interpretable way of defining expression data and will also be facilitated by other transformation methods such as the boxcox method which is also implemented in the application. The boxcox transformation developed 1964 by Box and Cox citep\{Sakia1992} applies a recursion on the data by determining the parameter lambda. Depending on the size of lambda a certain function will be applied on each value of the expression set in order to introduce normality. As already mentioned transformations applied by the BoxCox method range from log10 to taking the square root of each value
