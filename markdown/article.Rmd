...
Preparation for hypothesis testing involves data transformation and normalization. Missing values for proteins in individual channels are handled according to user selection with methods such as the K-nearest neighbor algorithm (=KNN) \citep{Lan2013}. Before statistical analysis, dimensionality reduction techniques can be applied and observed with or without correcting for batch effect by ComBat \citep{Johnson2007}. Furthermore data processing can be validated with an entropy based approach for the conservation of information. 
...
\begin{scriptsize}
\begin{itemize}
\item Overview showing the individual processing steps of the analysis pipeline
\item Barchart showing the number of peptide spectrum matches, peptides and proteins
\item Mutual information intra- and inter- groupwise
\item Reporter ion intensity with factor selection to highlight
\item Median coefficient of variation for razor proteins and correlation heatmap for protein expression
\item Dimensionality reduction analysis with principle component analysis (PCA) and Uniform Manifold Approximation and Projection (UMAP)
\item Feature wise output for an individual protein over the course of the pipeline.
\item Statistics module
\item Pathway enrichment
\item Protein set ontology enrichment 
\end{itemize}
\end{scriptsize}
