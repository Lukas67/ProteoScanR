...
As seen in figure \@ref(fig:ui-numbers-default) the aspects shown in the main panel include:
\spacing{1}
\begin{scriptsize}
\begin{itemize}
\item \textbf{Overview:} Summary showing the individual processing steps of the analysis pipeline
\item \textbf{Summary Barplot:} Barchart showing the number of peptide spectrum matches, peptides and proteins
\item \textbf{Normalization validation:} Mutual information intra- and inter- groupwise
\item \textbf{Reporter ion intensity:} Includes factor selection to create boxplots indicating a factor
\item \textbf{Median coefficient of variation and correlation:} Variation of peptides for razor proteins and correlation heatmap for protein expression
\item \textbf{Dimensionality reduction:} Analysis with principle component analysis (PCA) and Uniform Manifold Approximation and Projection (UMAP)
\item \textbf{Feature wise output:} Visualization of an individual protein over the course of the pipeline.
\item \textbf{Statistics module:} Differential expression analysis
\item \textbf{Pathway enrichment:} Over-representation analysis for differentially expressed proteins in the context of pathways.
\item \textbf{Protein set ontology enrichment:}  Over-representation analysis for differentially expressed proteins in a user selected context.
\end{itemize}
\end{scriptsize}
\spacing{1.5}
...
After performing a logarithmic transformation to the base 2 and quantile normalization, the mutual information (MI) within the healthy control (HC) group's sample type exhibits a minor reduction in the interquartile range (IQR) (as seen in figure \@ref(fig:ui-mutual-info-quant-norm)). However, it should be noted that there is an increase in the number of outliers compared to the raw data.
When switching to the quantile normalization the loss of mutual information between sample types decreased (as seen in figure \@ref(fig:ui-mutual-info-quant-norm)). Therefore the advised normalization technique for this particular data set would be the quantile normalization compared to the default method. 

\newpage
```{r ui-stat-HC-HIV-noMets, echo=F, out.width="70%", fig.align='center', fig.cap="Statistics module in portrait mode"}
