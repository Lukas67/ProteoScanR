...
```
Comparing sample types in regards of reporter ion intensity is recommended to observe any experimental bias such as batch/channel confounding effects which would influence the statistical model afterwards.
...
```
The distribution of median coefficient of variation (CV) shows the uniqueness in peptides for their respective razor peptides. This metric represents the presence or absence of noisy matches in the data set.
...
```
Razor peptide ambiguity over multiple batches can indicate experimental failures during sample prep or during acquisition. 
